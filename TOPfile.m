...
QPSK_ch_out = conv(ch,QPSK_filtered_sym)
QPSK_mul_ch_out = conv(multi_ch,QPSK_filtered_sym)
QAM_ch_out = conv(multi_ch,QAM_filtered_sym)
QAM_mul_ch_out = conv(multi_ch,QAM_filtered_sym)


for SNR_i  = -100:10:100

    QPSK_single_noise = addGaussianNoise(QPSK_ch_out,SNR_i);
    QPSK_multi_noise = addGaussianNoise(QPSK_mul_ch_out,SNR_i);
    QAM_single_noise = addGaussianNoise(QAM_ch_out,SNR_i);
    QAM_multi_noise = addGaussianNoise(QAM_mul_ch_out,SNR_i);
...
%------------------receiver------------------------------
   ...
   %------------------delay 제거------------------------------
   QPSK_single_recv_sym = QPSK_single_recv_sym(49:425);
   QPSK_multi_recv_sym = QPSK_multi_recv_sym(49:425);
   QAM_single_recv_sym = QAM_single_recv_sym(49:425);
   QAM_multi_recv_sym = QAM_multi_recv_sym(49:425);
   
     %-------------------receiving sampling-----------------
    QPSK_single_sampled_sym = recv_sym(1:8:8*length(sym_idx));
    QPSK_multi_sampled_sym =recv_sym(1:8:8*length(sym_idx));
    QAM_single_sampled_sym = recv_sym(1:8:8*length(sym_idx));
    QAM_multi_sampled_sym = recv_sym(1:8:8*length(sym_idx));
...
QAM_single_rx_bit_out = reshape(QAM_single_rx_bit_out, [12, 8]); % 처음 msg의 matrix 차원으로 변경 
QAM_multi_rx_bit_out = reshape(QAM_multi_rx_bit_out, [12, 8]); % 처음 msg의 matrix 차원으로 변경 
QPSK_single_rx_bit_out = reshape(QPSK_single_rx_bit_out, [12, 8]); % 처음 msg의 matrix 차원으로 변경 
QPSK_multi_rx_bit_out = reshape(QPSK_multi_rx_bit_out, [12, 8]); % 처음 msg의 matrix 차원으로 변경 
...
for idx = 1:size(QAM_single_recv_bits,1)
    [q,s] = gfdeconv(fliplr(QAM_single_recv_bits(idx,:)),fliplr(G));
    while size(s) ~= size(G)-1;
        s(end+1) = 0;
    end
    s = fliplr(s);
    ...
    if isequal(s,[1 1 1 0])
        disp('1 activatied');
            QAM_single_recv_bits(idx,1) = QAM_single_recv_bits(idx,1) + 1;
            QAM_single_recv_bits(idx,1) = rem(QAM_single_recv_bits(idx,1),2);
    elseif  isequal(s,[0 1 1 1])   
        disp('2 activatied')
            QAM_single_recv_bits(idx,2) = QAM_single_recv_bits(idx,2) + 1;
            QAM_single_recv_bits(idx,2) = rem(QAM_single_recv_bits(idx,2),2);
    elseif  isequal(s,[1 1 0 1])     
        disp('3 activatied')
             QAM_single_recv_bits(idx,3) = QAM_single_recv_bits(idx,3) + 1;
            QAM_single_recv_bits(idx,3) = rem(QAM_single_recv_bits(idx,3),2);
    elseif  isequal(s,[1 0 0 0])  
        disp('4 activatied')
            QAM_single_recv_bits(idx,4) = QAM_single_recv_bits(idx,4) + 1;
            QAM_single_recv_bits(idx,4) = rem(QAM_single_recv_bits(idx,4),2);
    elseif  isequal(s,[0 1 0 0])     
        disp('5 activatied')
           QAM_single_recv_bits(idx,5) = QAM_single_recv_bits(idx,5) + 1;
            QAM_single_recv_bits(idx,5) = rem(QAM_single_recv_bits(idx,5),2);
    elseif  isequal(s,[0 0 1 0])   
        disp('6 activatied')
            QAM_single_recv_bits(idx,6) = QAM_single_recv_bits(idx,6) + 1;
            QAM_single_recv_bits(idx,6) = rem(QAM_single_recv_bits(idx,6),2);
    elseif  isequal(s,[0 0 0 1])    
        disp('7 activatied')
            QAM_single_recv_bits(idx,7) = QAM_single_recv_bits(idx,7) + 1;
            QAM_single_recv_bits(idx,7) = rem(QAM_single_recv_bits(idx,7),2);
    elseif  isequal(s,[0 0 1 1])   
        disp('8 activatied')
            QAM_single_recv_bits(idx,1) = QAM_single_recv_bits(idx,1) + 1;
            QAM_single_recv_bits(idx,1) = rem(QAM_single_recv_bits(idx,1),2);
 ...
 QAM_single_recv_msg = QAM_single_recv_bits(:,[end-(k-1):end]);   

for idx = 1:size(QAM_multi_recv_bits,1)
    [q,s] = gfdeconv(fliplr(QAM_multi_recv_bits(idx,:)),fliplr(G));
    while size(s) ~= size(G)-1;
        s(end+1) = 0;
    end
    s = fliplr(s);
    ...
    if isequal(s,[1 1 1 0])
        disp('1 activatied');
            QAM_multi_recv_bits(idx,1) = QAM_multi_recv_bits(idx,1) + 1;
            QAM_multi_recv_bits(idx,1) = rem(QAM_multi_recv_bits(idx,1),2);
    elseif  isequal(s,[0 1 1 1])   
        disp('2 activatied')
            QAM_multi_recv_bits(idx,2) = QAM_multi_recv_bits(idx,2) + 1;
            QAM_multi_recv_bits(idx,2) = rem(QAM_multi_recv_bits(idx,2),2);
    elseif  isequal(s,[1 1 0 1])     
        disp('3 activatied')
             QAM_multi_recv_bits(idx,3) = QAM_multi_recv_bits(idx,3) + 1;
            QAM_multi_recv_bits(idx,3) = rem(QAM_multi_recv_bits(idx,3),2);
    elseif  isequal(s,[1 0 0 0])  
        disp('4 activatied')
            QAM_multi_recv_bits(idx,4) = QAM_multi_recv_bits(idx,4) + 1;
            QAM_multi_recv_bits(idx,4) = rem(QAM_multi_recv_bits(idx,4),2);
    elseif  isequal(s,[0 1 0 0])     
        disp('5 activatied')
           QAM_multi_recv_bits(idx,5) = QAM_multi_recv_bits(idx,5) + 1;
            QAM_multi_recv_bits(idx,5) = rem(QAM_multi_recv_bits(idx,5),2);
    elseif  isequal(s,[0 0 1 0])   
        disp('6 activatied')
            QAM_multi_recv_bits(idx,6) = QAM_multi_recv_bits(idx,6) + 1;
            QAM_multi_recv_bits(idx,6) = rem(QAM_multi_recv_bits(idx,6),2);
    elseif  isequal(s,[0 0 0 1])    
        disp('7 activatied')
            QAM_multi_recv_bits(idx,7) = QAM_multi_recv_bits(idx,7) + 1;
            QAM_multi_recv_bits(idx,7) = rem(QAM_multi_recv_bits(idx,7),2);
    elseif  isequal(s,[0 0 1 1])   
        disp('8 activatied')
            QAM_multi_recv_bits(idx,1) = QAM_multi_recv_bits(idx,1) + 1;
            QAM_multi_recv_bits(idx,1) = rem(QAM_multi_recv_bits(idx,1),2);
 ...
 QAM_mulit_recv_msg = QAM_mulit_recv_bits(:,[end-(k-1):end]);   
 ...
 QAM_mulit_output_reg = Scrambler(QAM_mulit_recv_msg,k);
 
 
 QPSK_single_output = bin2dec(num2str(QPSK_single_output_reg));
 QPSK_multi_output = bin2dec(num2str(QPSK_multi_output_reg));
 QAM_single_output = bin2dec(num2str(QAM_single_output_reg));
 QAM_mulit_output = bin2dec(num2str(QAM_mulit_output_reg));
 
 
disp('descrambled reg_output');
output_reg = Scrambler(recv_msg,k);
output = bin2dec(num2str(output_reg));
disp(output);
